...
// const coverToBeDisabled = ref(null)
/*TABS*/
...
const handleDeleteCover = (item) => {

  deleteCoverModal.value = true
  policyToBeDisabled.value = item
}
...
const rules = {
  ...
  administration_fee: [(v) => !!v || 'Administration Fee is required.', (v) => /^\d+$/.test(v) || 'Max. Entry Age must be a numeric value.'],
};
...
const handleSaveProduct = async () => {
  ...
  if (!form.administration_fee) {
    addErrorFields.administration_fee = true;
  }
  ...
  try {
    ...
    const requestData = {
      ...
      date: formatDate(),
      administration_fee: form.administration_fee,
...
const showUpdateDialog = (item) => {
  ...
  forms.administration_fee = item.administration_fee;
...
const handleUpdateProduct = async () => {
  ...
  if (!forms.administration_fee) {
    errorFields.administration_fee = true;
  }
  if (!policyCovers.length) {
  ...
  try {
    ...
    const requestUpdateData = {
      ...
      date: formatDate(),
      administration_fee: forms.administration_fee,
  ...
  <VCard class="text-center text-sm-start">
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12" sm="6" md="4">
                <v-textarea v-model="form.administration_fee" label="Administration Fee*"
                  :rules="rules.administration_fee"></v-textarea>
              </v-col>
            </v-row>
            ...
            <v-row v-for="(cover, index) in policyCovers" :key="index">
              <v-col cols="12" sm="6" md="8" v-for="(field, fieldIndex) in formFields" :key="fieldIndex">
                <v-text-field v-model="cover[field.model]" :label="field.label" :rules="field.rules"></v-text-field>
              </v-col>
              <v-col cols="auto">
    ...
    <v-dialog v-model="deleteCoverModal" max-width="600px">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue-darken-1" variant="text" @click="deleteCoverModal = false">Cancel</v-btn>
          <v-btn color="blue-darken-1" variant="text" @click="disablePolicy">OK</v-btn>
          <v-spacer></v-spacer>
        </v-card-actions>
      </v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12" sm="6" md="4">
                <v-textarea v-model="forms.administration_fee" label="Administration Fee*" required
                  :rules="rules.administration_fee" @scroll="handleInputScroll" @focus="handleInputFocus"
                  @blur="handleInputBlur"></v-textarea>
              </v-col>
              ...
              <v-table>
                ...
                <tbody>
                  <tr v-for="(cover, index) in forms.policy_covers" :key="cover.sid" class="clickable-row">
                    ...
                    <td class="text-center">
                      ...
                      <template v-else>
                        <v-text-field v-model="cover.premium" label="Premium*" outlined></v-text-field>
                      </template>
                    </td>
                    <td class="text-center">
                      <template v-if="editIndex !== index">
                      ...
                      <template v-else>
                        <v-text-field v-model="cover.underwriter" label="Underwriter*" outlined></v-text-field>
                      </template>
                    </td>
                    <td class="text-center">
                      <template v-if="editIndex !== index">
                      ...
                      <template v-else>
                        <v-text-field v-model="cover.max_entry_age" label="Max Entry Age*" outlined></v-text-field>
                      </template>
                    </td>
                    <td class="text-center">
                      <template v-if="editIndex !== index">
                        ...
                        <!-- <v-col cols="auto"> -->
                        <v-btn @click="handleDeleteCover(index.sid)">Remove</v-btn>
                        <!-- </v-col> -->
                      </template>
                      <template v-else>
