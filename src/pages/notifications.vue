    ...
    try {
      ...
      if (response && response.status === 200) {
        ...
        response.data.forEach((message) => {
          // You can use a combination of fields to determine uniqueness
        ...
        // Convert the object to an array if you need an array format
  ...
  <v-table>
    ...
    <tbody>
      ...
      >
        ...
        <td>{{ item.flagCounts[0] }}/{{ item.count }}</td>
        <td>{{ item.flagCounts[1] }}/{{ item.count }}</td>
        <td>{{ item.flagCounts[2] }}/{{ item.count }}</td>
        <td>{{ item.flagCounts[3] }}/{{ item.count }}</td>
